 ...
 *
 * DESCRIPTION
 *   Compare strings @str and @expect and return 1 if they are the same. 문자열 같으면 1 리턴
 ...
 * | ------ | --------- | ----------------------- |
 * | `add`  | r-format  | 0 + 0x20 ==               |
 * | `addi` | i-format  | 0x08    i                |
 * | `sub`  | r-format  | 0 + 0x22  ==              |
 * | `and`  | r-format  | 0 + 0x24  ==              |
 * | `andi` | i-format  | 0x0c    i                |
 * | `or`   | r-format  | 0 + 0x25   ==             |
 * | `ori`  | i-format  | 0x0d    i                |
 * | `nor`  | r-format  | 0 + 0x27  ==              |
 * | `sll`  | r-format  | 0 + 0x00   ==             |
...
{    if(instr == 0xffffffff ){return 0;}
    int op = instr >> 26;
    if(op == 0){//r-format
    int rs = (instr >> 21) & 0b11111;
    int rt = (instr >> 16) & 0b11111;
    int rd = (instr >> 11) & 0b11111;
    ...
    if(funct == 0x20){
        ...
        if((signed int)registers[rt]<0){
            int expansion = (0xFFFFFFFF >> shamt-1 ) << 31;
            registers[rd] = registers[rd] | expansion;
        }
        //registers[rd] = registers[rt] >> shamt;
        // int bit = (registers[rt] & 0b10000) >> 4;
        // printf("%d", bit); /// =================================================
        return 1;
        ...
        //addi
        //printf("addi\n");
        int rs = (instr & 0b11111000000000000000000000) >> 21;
        int rt = (instr & 0b111110000000000000000) >> 16;
        int imm = (instr & 0b1111111111111111);
        if(imm&0b1000000000000000==1){
            imm = 0b11111111111111110000000000000000|imm;
        }
        ...
        if(imm&0b1000000000000000==1){
         ...
         int one = memory[registers[rs]+imm++];
        one = one << 24;
         int two = memory[registers[rs]+imm++];
        two = two << 16;
         int three = memory[registers[rs]+imm++];
        three = three << 8;
         int four = memory[registers[rs]+imm++];
        registers[rt] = one | two | three | four;
        ...
        memory[registers[rs]+imm++] = (registers[rt] & 0b11111111000000000000000000000000) >> 24;
        memory[registers[rs]+imm++] = (registers[rt] & 0b111111110000000000000000) >> 16;
        memory[registers[rs]+imm++] = (registers[rt] & 0b1111111100000000) >> 8;
        memory[registers[rs]+imm] = registers[rt] & 0b11111111;
        ...
        if(registers[rs] == registers[rt]){
            if(imm&0b1000000000000000==1){
            imm = 0b11111111111111000000000000000000 + (imm << 2);
            }
            else{imm = imm << 2;}
            pc = pc + imm;
            //=======================라벨로 점프, pc값 변경
        ...
        //j
        int address = instr & 0b11111111111111111111111111;
