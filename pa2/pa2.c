...
static int process_instruction(unsigned int instr)
{
	...
	char type;
	int opcode=instr >> 26;
	int rs, rt, rd, shamt, funct, address;
	int sign;
	if(opcode!=0){//i-format, j-format
	rs=(instr >> 21) & 0b11111;
	rt = (instr >> 16) & 0b11111;
	address = instr & 0b1111111111111111;
	if((address>>15)==1&&(address>>12)==0xf) sign=1;
	else sign=0;	
	type='i';
	if(sign==1){
	address=(~(address-1)<<16)>>16;
	}
	//printf("%d %d %d %d %x\n",sign,opcode,rs,rt,address);
	if(opcode==0x02||opcode==0x03){//j-format
	address= (instr << 6) >> 6;
	type='j';
	}
	}
	...
	int tmp=registers[rs]+address;
	registers[rt]=(memory[tmp]<<24)+(memory[tmp+1]<<16)+(memory[tmp+2]<<8)+memory[tmp+3];
	}
	else if(opcode==0x2b){//sw
	int tmp=registers[rs]+address;
	memory[tmp]=registers[rt]>>24;
	memory[tmp+1]=(registers[rt]<<8)>>24;
	memory[tmp+2]=(registers[rt]<<16)>>24;
	memory[tmp+3]=registers[rt]&0b11111111;
	}
...
static int load_program(char * const filename)
{
	...
	int num=strtoimax(arr,NULL,0);
	memory[tmp]=num>>24;
	memory[tmp+1]=(num>>16)&0b11111111;
	memory[tmp+2]=(num>>8)&0b11111111;
	memory[tmp+3]=num&0b11111111;
	tmp=tmp+4;
	//int tmp=process_instruction(num);
	if(tmp==0)
		return 0;
	}
	}
...
static int run_program(void)
{
	...
	int tmp=1;
	while(tmp==1){
	num=(memory[pc]<<24)+(memory[pc+1]<<16)+(memory[pc+2]<<8)+memory[pc+3];
	pc+=4;	
	tmp=process_instruction(num);
	}
